...
export namespace std
{
    ...
    [[nodiscard]] constexpr _Decode_result<char> _Decode_utf(const char* first, const char* last, char32_t& _Val) noexcept {
        _Val = static_cast<char32_t>(static_cast<unsigned char>(*first));
        int _Num_bytes;
        if (_Val <= 0x7F)
            return { first + 1, true };
        ...
        for (int _Idx = 1; _Idx < _Num_bytes; ++_Idx)
        {
            ...
            _Val = (_Val << 6) | (static_cast<unsigned char>(first[_Idx]) & 0b11'1111u);
        }
