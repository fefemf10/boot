...
export namespace std
{
	...
	struct pair
	{
		...
		constexpr explicit(!conjunction_v<is_convertible<_Other1&, T1>, is_convertible<_Other2&, T2>>) pair(pair<_Other1, _Other2>& _Right) noexcept(is_nothrow_constructible_v<T1, _Other1&>&& is_nothrow_constructible_v<T2, _Other2&>) : first(_Right.first), second(_Right.second) {}
		...
		constexpr explicit(!conjunction_v<is_convertible<const _Other1&, T1>, is_convertible<const _Other2&, T2>>) pair(const pair<_Other1, _Other2>& _Right) noexcept(is_nothrow_constructible_v<T1, const _Other1&>&& is_nothrow_constructible_v<T2, const _Other2&>) : first(_Right.first), second(_Right.second) {}

		template <class _Other1, class _Other2, enable_if_t<conjunction_v<is_constructible<T1, _Other1>, is_constructible<T2, _Other2>>, int> = 0>
		constexpr explicit(!conjunction_v<is_convertible<_Other1, T1>, is_convertible<_Other2, T2>>) pair(pair<_Other1, _Other2>&& _Right) noexcept(is_nothrow_constructible_v<T1, _Other1>&& is_nothrow_constructible_v<T2, _Other2>) : first(forward<_Other1>(_Right.first)), second(forward<_Other2>(_Right.second)) {}

		template <class _Other1, class _Other2, enable_if_t<conjunction_v<is_constructible<T1, const _Other1>, is_constructible<T2, const _Other2>>, int> = 0>
		constexpr explicit(!conjunction_v<is_convertible<const _Other1, T1>, is_convertible<const _Other2, T2>>) pair(const pair<_Other1, _Other2>&& _Right) noexcept(is_nothrow_constructible_v<T1, const _Other1>&& is_nothrow_constructible_v<T2, const _Other2>) : first(forward<const _Other1>(_Right.first)), second(forward<const _Other2>(_Right.second)) {}
		...
		constexpr explicit(!conjunction_v<is_convertible<decltype(get<0>(declval<_Other>())), T1>, is_convertible<decltype(get<1>(declval<_Other>())), T2>>) pair(_Other&& _Right) noexcept(is_nothrow_constructible_v<T1, decltype(get<0>(declval<_Other>()))>&& is_nothrow_constructible_v<T2, decltype(get<1>(declval<_Other>()))>) : first(get<0>(forward<_Other>(_Right))), second(get<1>(forward<_Other>(_Right))) {}
		...
		constexpr pair& operator=(_Identity_t<const _Myself&> _Right) noexcept(conjunction_v<is_nothrow_copy_assignable<T1>, is_nothrow_copy_assignable<T2>>)
		{
			first = _Right.first;
		...
		template <class _Myself = pair,
			...
			first = forward<T1>(_Right.first);
			second = forward<T2>(_Right.second);
			return *this;
		}
		...
		{
			first = get<0>(forward<_Other>(_Right));
			second = get<1>(forward<_Other>(_Right));
			return *this;
		}
		...
		constexpr void swap(pair& _Right) noexcept(_Is_nothrow_swappable<T1>::value&& _Is_nothrow_swappable<T2>::value)
		{
		...
		template <int = 0>
		constexpr void swap(const pair& _Right) const noexcept(is_nothrow_swappable_v<const T1>&& is_nothrow_swappable_v<const T2>)
		{
			if (this != addressof(_Right))
			{
				swap(first, _Right.first);
